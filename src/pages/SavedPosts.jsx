    ...
    <div className="space-y-6">
      ...
      <div className="bg-brand-surface rounded-3xl p-6 border border-brand-border shadow-soft-sm flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-brand-mainText flex items-center gap-2">
              <Bookmark className="w-5 h-5 text-brand-purple fill-brand-purple" />
              <span>Saved Posts</span>
            </h2>
            <p className="text-xs text-brand-mutedText mt-0.5">Posts you've bookmarked for later</p>
          </div>
        </div>
      </div>
      ...
      {loading ? (
        <div className="space-y-6">
          {[1, 2].map((n) => (
            <div key={n} className="bg-brand-surface rounded-3xl p-6 border border-brand-border shadow-soft-sm animate-pulse space-y-4">
              <div className="flex items-center gap-3">
                <div className="w-11 h-11 bg-brand-lavender rounded-full" />
                <div className="space-y-2 flex-1">
                  <div className="w-32 h-4 bg-brand-lavender rounded" />
                  <div className="w-20 h-3 bg-brand-lavender rounded" />
                </div>
              </div>
              <div className="h-16 bg-brand-lavender rounded-2xl" />
            </div>
          ))}
        </div>
      ) : posts.length === 0 ? (
        <div className="bg-brand-surface rounded-3xl p-12 border border-brand-border shadow-soft-sm text-center flex flex-col items-center justify-center space-y-3">
          ...
          <h3 className="font-bold text-lg text-brand-mainText">No saved posts yet</h3>
