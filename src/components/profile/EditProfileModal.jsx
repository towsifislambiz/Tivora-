...
export default function EditProfileModal({ isOpen, onClose, onShowToast }) {
  ...
  return (
    <div class="fixed inset-0 z-50 bg-brand-mainText/50 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
      <div class="bg-brand-surface rounded-3xl w-full max-w-lg shadow-soft-lg border border-brand-border overflow-hidden animate-in fade-in zoom-in duration-200 my-8">
        {/* Header */}
        <div class="flex items-center justify-between px-6 py-4 border-b border-brand-border">
          <h3 class="font-bold text-lg text-brand-mainText">Edit Tivora Profile</h3>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} class="p-6 space-y-5">
          ...
          <div class="space-y-1.5 bg-brand-lavender/60 p-3.5 rounded-2xl border border-brand-border">
            <div class="flex items-center justify-between">
              <label class="text-xs font-bold text-brand-mainText flex items-center gap-1.5">
                <span>Unique Tivora Profile ID</span>
                <Lock class="w-3.5 h-3.5 text-brand-purple" />
              </label>
              <span class="text-[0.65rem] font-bold text-brand-purple bg-brand-purple/10 px-2 py-0.5 rounded-full">
                PERMANENT
              </span>
            </div>
            <p class="text-xs font-bold text-brand-purple">@{permanentUsername}</p>
            <p class="text-[0.68rem] text-brand-mutedText">
          ...
          <div class="space-y-2">
            <label class="text-xs font-semibold text-brand-mainText block">Profile Picture</label>
            <div class="flex items-center gap-4">
              {photoURL ? (
                <img src={photoURL} alt="Profile" class="w-14 h-14 rounded-full object-cover border-2 border-brand-purple shadow-soft-sm" />
              ) : (
                <div class="w-14 h-14 rounded-full bg-primary-gradient text-white flex items-center justify-center font-bold text-lg shadow-soft-sm">
                  {displayName ? displayName.slice(0, 2).toUpperCase() : 'TV'}
                </div>
              )}
              <div class="flex-1 space-y-1">
                <label class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary-gradient text-white font-bold text-xs cursor-pointer shadow-gradient-glow hover:scale-105 transition-all">
                  {optimizingProfile ? (
                    <>
                ...
                <p class="text-[0.68rem] text-brand-mutedText">Auto-compressed & scaled to 400x400 for super-fast loading.</p>
              </div>
            </div>
          </div>
          ...
          <div class="space-y-2">
            <label class="text-xs font-semibold text-brand-mainText block">Cover Photo</label>
            <div class="space-y-2">
              <div class="h-28 rounded-2xl bg-cover-gradient overflow-hidden relative border border-brand-border shadow-soft-sm">
                {coverPhotoURL ? (
                  <img src={coverPhotoURL} alt="Cover" class="w-full h-full object-cover" />
                ) : (
                  <div class="w-full h-full bg-cover-gradient opacity-90 flex items-center justify-center text-xs text-white/80 font-medium">
                    Gradient Cover Fallback
                  </div>
                )}
              </div>
              <label class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-brand-lavender text-brand-purple hover:bg-brand-purple/10 font-bold text-xs cursor-pointer transition-all border border-brand-purple/30">
                {optimizingCover ? (
                  <>
                    <Loader2 class="w-4 h-4 animate-spin" />
              ...
              <p class="text-[0.68rem] text-brand-mutedText">Auto-compressed to 1200x500 for lightning fast page loads.</p>
            </div>
          </div>

          {/* 5. Bio */}
          <div class="space-y-1.5">
            <label class="text-xs font-semibold text-brand-mainText block">Bio</label>
            <div class="relative">
              ...
              <textarea
                ...
                class="w-full min-h-[90px] bg-brand-lavender border border-transparent focus:border-brand-purple focus:bg-white rounded-2xl pl-11 pr-4 py-2.5 text-xs sm:text-sm text-brand-mainText outline-none transition-all resize-none"
              />
            </div>
          </div>
          ...
          {/* 6. Location */}
          <div class="space-y-1.5">
            <label class="text-xs font-semibold text-brand-mainText block">Location</label>
            <div class="relative">
          ...
          <div class="space-y-2">
            <label class="text-xs font-semibold text-brand-mainText block">Hobbies & Interests</label>
            <div class="flex flex-wrap gap-2 mb-2">
              {hobbies.map((hobby, idx) => (
                <span
                  key={idx}
                  class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-brand-lavender text-brand-purple text-xs font-semibold border border-brand-purple/20"
                >
                  <span>{hobby}</span>
                  <button
                    type="button"
                    onClick={() => handleRemoveHobby(idx)}
                    class="w-3.5 h-3.5 rounded-full hover:bg-brand-purple/20 flex items-center justify-center text-brand-purple"
                  >
                    <X class="w-2.5 h-2.5" />
                  </button>
                </span>
            ...
            <div class="flex items-center gap-2">
              <input
                ...
                class="flex-1 h-9 bg-brand-lavender border border-transparent focus:border-brand-purple focus:bg-white rounded-full px-4 text-xs text-brand-mainText outline-none"
              />
              <button
                type="button"
                onClick={handleAddHobby}
                class="h-9 px-4 rounded-full bg-brand-purple text-white font-bold text-xs hover:bg-brand-purple/90 flex items-center gap-1 shrink-0"
              >
          ...
          <div class="pt-3 border-t border-brand-border flex items-center justify-end gap-3">
            ...
            <button
              ...
              class="px-6 py-2 rounded-full bg-primary-gradient text-white font-semibold text-xs shadow-gradient-glow hover:scale-105 transition-transform disabled:opacity-50"
            >
